//
//  ArchiveHandlerCompressTests.swift
//  MacPacker
//
...
final class ArchiveHandlerCompressTests: XCTestCase {
    let testBase: ArchiveTestBase = ArchiveTestBase()
    let defaultTestFile: String = "defaultArchive.tar.Z"
    ...
    func testExtractFile() throws {
        ...
        if let se = archive.currentStackEntry {
            XCTAssertEqual(se.name, "defaultArchive.tar")
        }
        ...
        let url = archive.extractFileToTemp(archive.items[3])
        XCTAssertNotNil(url)
    ...
    func testExtractFullArchive() throws {
        ...
        XCTAssertTrue(testBase.fileExistsInTemp("defaultArchive.tar"))
    }
}
